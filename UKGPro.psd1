...
Cmdlets: Connect-UKGPro (with new -FromVault and -FromEnvironment
parameter sets), Disconnect-UKGPro, Save-UKGProCredential and
Update-UKGProCredential (one-time SecretManagement setup + partial
rotation; hostname + tenant API keys only -- username/password are
never persisted in the vault by design), plus
...
Unified auth (Basic + US-Customer-API-Key + x-api-key), automatic
page/per_Page pagination, friendly date filters for UKG's
operator-prefixed date syntax. Every Get- cmdlet works with a
View-only UKG service account.
'@
        }
    }
}
