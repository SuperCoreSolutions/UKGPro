function Disconnect-UKGPro {
    ...
    .DESCRIPTION
        Removes the stored session (Basic token, customer api key, user api key,
        base URL) from module state. UKG Pro uses stateless per-request auth, so
        there is no server-side token to revoke — this simply clears local
        credentials.
